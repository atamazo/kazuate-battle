...
def session_ready_for(phase: str) -> bool:
    """Return True if minimum required keys exist for the given phase."""
    if phase == 'setup':
        return True
    if phase == 'secrets1':
        return all(k in session for k in ('p1_name', 'p2_name', 'eff_num_min', 'eff_num_max'))
    if phase == 'secrets2':
        return all(k in session for k in ('p1_name', 'p2_name', 'eff_num_min', 'eff_num_max', 'secret1'))
    if phase == 'play':
        needed = ('p1_name','p2_name','secret1','secret2','hidden_secret','turn',
                  'available_hints','hint_choice_available','cooldown',
                  'trap_kill','trap_info','pending_view','can_view','view_cut_index',
                  'skip_next_turn','actions_log','score1','score2','round_no')
        return all(k in session for k in needed)
    if phase == 'end_round':
        return all(k in session for k in ('p1_name','p2_name','winner','score1','score2','round_no','secret1','secret2','hidden_secret','target_points'))
    return False
...
def full_game():
    ...
    # If the client has a stale/broken cookie from a previous version, reset cleanly
    if not session_ready_for(phase):
        reset_to_setup()
        phase = 'setup'
...
def on_500(e):
    # Clear broken session and guide user back to top
    try:
        reset_to_setup()
    except Exception:
        session.clear()
